...
def create_app():
    ...
    # These need to be mutable objects to be shared between functions
    class SharedState:
        def __init__(self):
    ...
    with gr.Blocks(title="Ollama Model Comparator", theme=get_theme()) as app:
        ...
        # Define an update function that will be called periodically
        def update_ui():
            # Only update if we're not in "stop" mode
            if shared_state.stop_updates:
        ...
        # Set up the timer for UI updates - this will poll every 0.5 seconds
        update_timer = gr.Textbox(visible=False)  # Hidden element to trigger updates
        
        def update_timer_fn():
            # This will run every 0.5 seconds when active
            return "update"
        
        # Initialize the timer but don't start it yet
        timer = gr.Timer(0.5, update_timer_fn, update_timer)
        
        # When the timer updates, propagate those updates to the UI
        update_timer.change(
            update_ui,
            inputs=[],
        ...
        def clear_outputs():
            ...
            # Stop updates
            shared_state.stop_updates = True
        ...
        def compare_models(model1_name, model2_name, sys_prompt, user_prompt, temp, max_tok, timeout_sec):
            # Reset and prepare shared state
            with shared_state.model1_lock:
            ...
            # Enable UI updates
            shared_state.stop_updates = False
