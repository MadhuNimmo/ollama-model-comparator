...
def create_app():
    ...
    with gr.Blocks(title="Ollama Model Comparator", theme=get_theme()) as app:
        ...
        # Create shared data for thread communication
        class SharedData:
            def __init__(self):
                self.results = {}
        ...
        def compare_models(model1_name, model2_name, sys_prompt, user_prompt, temp, tokens, timeout_sec, threads):
            # Stop any running threads (cannot actually stop them but this is a placeholder)
            ...
            # Create callbacks for updating the UI - these need to update components directly
            # without using the queue() method
            def update_model1(model_name, text, tokens, time_elapsed):
                # Instead of using app.queue(), we'll use gr.update() which will be handled in the next yielded update
                output = update_model_output(0, model_name, text, tokens, time_elapsed)
                # Store the model results in shared data
                shared_data.update_result(model_name, {
                    "response": text,
                    "elapsed_time": round(time_elapsed, 2),
                    "token_count": tokens
                })
                
                # Use a direct approach to update the UI
                for key, value in output.items():
                    if hasattr(key, "update"):
                        key.update(value=value)
            
            def update_model2(model_name, text, tokens, time_elapsed):
                # Similar approach for model 2
                output = update_model_output(1, model_name, text, tokens, time_elapsed)
            ...
            def run_model1():
                ...
                # Final update to ensure we have complete results
                shared_data.update_result(model1_name, result)
            ...
            def run_model2():
                ...
                # Final update
                shared_data.update_result(model2_name, result)
