...
def create_app():
    ...
    with gr.Blocks(title="Ollama Model Comparator", theme=get_theme()) as app:
        ...
        def compare_models(model1_name, model2_name, sys_prompt, user_prompt, temp, tokens, timeout_sec, threads):
            ...
            def update_model1(text, tokens, time_elapsed):
                app.queue(lambda: update_model_output(0, model1_name, text, tokens, time_elapsed))
            
            def update_model2(text, tokens, time_elapsed):
                app.queue(lambda: update_model_output(1, model2_name, text, tokens, time_elapsed))
            ...
            def run_model1():
                ...
                # Store final result
                app.queue(lambda: gr.State.update(value={**model_results.value, model1_name: result}), outputs=[model_results])
            ...
            def run_model2():
                ...
                # Store final result
                app.queue(lambda: gr.State.update(value={**model_results.value, model2_name: result}), outputs=[model_results])
